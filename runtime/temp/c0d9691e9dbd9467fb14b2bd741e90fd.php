<?php /*a:3:{s:62:"E:\phpStudy\WWW\jusha\application\admin\view\category\add.html";i:1545451660;s:61:"E:\phpStudy\WWW\jusha\application\admin\view\common\head.html";i:1545055853;s:61:"E:\phpStudy\WWW\jusha\application\admin\view\common\foot.html";i:1545055853;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="admin-main fadeInUp animated">
    ...
    <form class="layui-form layui-form-pane">
        <div class="layui-form-item">
            ...
            <div class="layui-input-3">
                <select name="moduleid" lay-verify="required" lay-filter="moduleid" id="moduleid">
                    <?php if(is_array($modulelist) || $modulelist instanceof \think\Collection || $modulelist instanceof \think\Paginator): $i = 0; $__LIST__ = $modulelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;if((isset($module['moduleid']))): ?>
                    <option value="<?php echo htmlentities($vo['id']); ?>" <?php if($module['moduleid'] == $vo['id']): ?>selected<?php endif; ?>><?php echo htmlentities($vo['title']); ?></option>
                    <?php else: ?>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">PC栏目图片</label>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">Mobile栏目图片</label>
            <input type="hidden" name="imageMobile" id="imageMobile">
            <div class="layui-input-block">
                <div class="layui-upload">
                    <button type="button" class="layui-btn layui-btn-primary" id="cateBtnMobile"><i class="icon icon-upload3"></i>点击上传</button>
                    <div class="layui-upload-list">
                        <img class="layui-upload-img" id="cateImageMobile">
                        <p id="demoTextMobile"></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                ...
                <input type="checkbox" name="readgroup[<?php echo htmlentities($i); ?>]" value="<?php echo htmlentities($item['group_id']); ?>" title="<?php echo htmlentities($item['title']); ?>">
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </div>
        </div>
        <div class="layui-form-item">
...
<script>
    ...
    layui.use(['form','upload'], function () {
        ...
        var uploadInstMobile = upload.render({
            elem: '#cateBtnMobile',
            url: '<?php echo url("UpFiles/upload"); ?>',
            done: function(res){
                if(res.code>0){
                    $('#imageMobile').val(res.url);
                    $('#cateImageMobile').attr('src', res.url);
                }else{
                    //如果上传失败
                    return layer.msg('上传失败');
                }
            ...
            error: function(){
                //演示失败状态，并实现重传
                var demoTextMobile = $('#demoTextMobile');
                demoTextMobile.html('<span style="color: #FF5722;">上传失败</span> <a class="layui-btn layui-btn-mini demo-reload">重试</a>');
                demoTextMobile.find('.demo-reload').on('click', function(){
                    uploadInst.upload();
                });
            }
        });
